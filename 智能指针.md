...
你需要的是一个原子操作检查`std::weak_ptr`是否已经过期，如果没有过期就访问所指对象。这**可以通过从`std::weak_ptr`创建`std::shared_ptr`来实现**，具体有两种形式可以从`std::weak_ptr`上创建`std::shared_ptr`，具体用哪种取决于`std::weak_ptr`过期时你希望`std::shared_ptr`表现出什么行为。一种形式是`std::weak_ptr::lock`，它返回一个`std::shared_ptr`，如果`std::weak_ptr`过期这个`std::shared_ptr`为空：
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMTcyMDI0NzYsLTg5MzA5MDgyOCwtMz
Y2MjMzMzMwLDE4MTAyNTQ3OTgsMjExNTc3MzQ2OCwxOTcwNDI0
NTgyLC0xMzAzNjk2NTI3LC0xMzI2NzE2MzksLTE5MDkxMDU4ND
csLTEzNDQwMTAzMzUsLTEzNzcyMzk5ODksLTMxNDM4NzI1OSwx
MTMyNzQ0NzgsLTEzMTUyNDQxMTEsLTE1OTc4MzIyNzAsNzMwOT
k4MTE2XX0=
-->