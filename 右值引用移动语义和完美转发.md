...
glvalue是**拥有身份**的表达式，它对应了一块内存地址。glvalue有lvalue和xvalue两种形式，具体的示例见下文。
...
rvalue是指**可以移动**的表达式。prvalue和xvalue都是rvalue，具体的示例见下文。
...
**变量，函数或者数据成员的名称**都是左值表达式。下面是一些左值的例子：

```
"hello world" // lvalue
int a{}; // lvalue
++a; // lvalue 注意 这个表达式是左值， 所以++a = 4 是合法的。

int& get() {return a;}
get(); // lvalue // 返回引用的函数调用返回左值。
...
int foo();
foo();// prvalue //注意返回非引用的函数调用是个纯右值。

int a{}, b{}; // both lvalues
a + b; // prvalue
&a; // prvalue

a++ // prvalue // 注意这里返回时右值， 所以 a++=4 是不合法的。
b-- // prvalue
a && b  // prvalue
a < b // prvalue
...
```
bool b {true}; // lvalue
std::move(b); // xvalue //move 把左值引用转成右值引用， 产生xvalue.
...
(2) **把一个对象转成右值引用的cast 操作**

-   例如常见的 `static_cast<T&&>(t)`

(3) **对于数组move后的下标访问操作**。

-   例如 `int a[5]; std::move(a)[0];`
-   **更准确**的说法：必须是 **built-in** subscript expression，因为 operator[] 是可以重载的

(4) **对于类的数据成员的move后的访问操作**
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUxMDU3OTEwOCwtMzg4MDY3ODQwLDEzNz
k0MzY1ODMsLTQ4Nzc5OTQ3LC00ODI0NTgxMjIsLTU0NzE1NjYy
OCwtMjM4NjMzOTkyLDgyNTM0OTU5LC0xMjU2MDI1MTIsLTk0MD
YzMzU2MiwtMjQ2NzU4NTgwLDE1MDY5ODc4OCwtMTQ0NTQyMjc5
MCw5NDE3NzM4MzQsLTE2NDgwNTU4MDksLTExODIyNDE0NTMsMT
IxOTExODg5Miw0NTg1MzM5NDAsLTk4OTU4MzA0NywtMzUxMjE3
NjEyXX0=
-->