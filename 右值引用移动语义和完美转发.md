...
```
在 Widget 的构造函数当中,  `rhs`  是一个rvalue reference，**前面提到，右值引用只能被绑定到右值上**，所以我们知道它被绑定到了一个rvalue上面(i.e., 因此我们需要传递了一个rvalue给它)， 但是  `rhs`  本身是一个 lvalue，所以，当我们想要用到这个被绑定在  `rhs`  上的rvalue 的 rvalueness 的时候，我们就需要把  `rhs`  转换回一个rvalue。之所以我们想要这么做，是因为我们想将它作为一个移动操作的source，这就是为什么我们**用  `std::move`将它转换回一个  `rvalue`**。
...
`Gadget`  构造函数当中的`rhs`  是一个 universal reference,，所以它可能绑定到一个 lvalue 或者 rvalue 上，但是无论它被绑定到什么东西上，`rhs`  本身还是一个 lvalue。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5ODMyNjUzNDIsOTcwNTIyOTc2LDExMj
k2NzYzNDEsLTIyMjY3Mzk5MCwtODUwMzYyODM1LC02NjM0Njcz
OSwtMTY3ODI4OTAzMiwtMTcyNzQzNzM0MSwtODQ4NDIzODIwLC
04MTI2MjY2OCwyNDkxNzY2ODAsLTgzMzAxODQ4NCwyMzY4NDYx
NjUsLTcyNzc0MTgzMiwtNzg4Nzc4OTA1LDE3NzU5Mjg2MjEsMT
c5ODY1NDgwNSwtMjAzMzk4MjU1NCwtMTk1OTcwNjIxMiwtNTUy
MDkxNzQ4XX0=
-->