...
-   **lvalue** (Left-hand-side value) - 左值：a glvalue that is not an xvalue
-   **rvalue** (Right-hand-side value) - 右值：is a prvalue or an xvalue.
-   **xvalue** (eXpiring value) - 将亡值：is a glvalue that denotes an object or bit-field whose resources can be reused （usually because it is near the end of its lifetime）.
-   **prvalue** (Pure rvalue) - 纯右值：an expression whose evaluation initializes an object or a bit-field, or computes the value of an operand of an operator, as specified by the context in which it appears, or an expression that has type cv void.
-   **glvalue** (Generalized lvalue) - 泛左值： an expression whose evaluation determines the identity of an object, bit-field, or function
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM2Njc1NjYyNiwtNTQ3MTU2NjI4LC0yMz
g2MzM5OTIsODI1MzQ5NTksLTEyNTYwMjUxMiwtOTQwNjMzNTYy
LC0yNDY3NTg1ODAsMTUwNjk4Nzg4LC0xNDQ1NDIyNzkwLDk0MT
c3MzgzNCwtMTY0ODA1NTgwOSwtMTE4MjI0MTQ1MywxMjE5MTE4
ODkyLDQ1ODUzMzk0MCwtOTg5NTgzMDQ3LC0zNTEyMTc2MTIsLT
EwMzQ3NzI2NDMsMTQ0NDE0OTI5NywtMTY4MDkxMzcxNyw1MDUy
OTU2OThdfQ==
-->